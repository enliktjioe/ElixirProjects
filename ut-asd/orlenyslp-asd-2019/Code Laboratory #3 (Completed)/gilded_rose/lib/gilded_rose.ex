defmodule GildedRose do
  ...
  defp update_quality(%{sell_in: sell_in} = item) when sell_in <= 0, do: update_quality(item, 2)
  defp update_quality(item), do: update_quality(item, 1)
  defp update_quality(%{name: name, sell_in: sell_in} = item, factor) do
    case name do
      "Backstage passes"<>_ -> cond do
        sell_in <= 0 -> %{item | quality: 0}
        sell_in <= 5 -> increase_quality(item, 3)
        sell_in <= 10 -> increase_quality(item, 2)
  ...
  defp update_item(%{name: "Sulfuras"<>_} = item), do: item
